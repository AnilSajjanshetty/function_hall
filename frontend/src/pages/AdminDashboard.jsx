...
import { bookingAPI, announcementAPI, eventAPI, messageAPI } from '../services/api';
import { Calendar, X, CheckCircle, Bell, PartyPopper, Mail, LogOut } from 'lucide-react';
...
const DEFAULT_RECENT_EVENTS = [
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const [bookingsData, eventsData, announcementsData, messagesData] = await Promise.all([
        bookingAPI.getAll(),
        eventAPI.getAll(),
  ...
  const handleAnnouncementSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Announcement published! Emails: ${response.emailsSent}, SMS: ${response.smsSent}`);
      ...
      <div className="container mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-2 flex gap-2 mb-6 overflow-x-auto">
          {[
            ...
            { id: 'gallery', label: 'Gallery', count: events.length, icon: '🖼️' },
            { id: 'announcements', label: 'Announcements', count: announcements.length, icon: '📢' },
            { id: 'messages', label: 'Messages', count: messages.length, icon: '📧' }
          ].map(tab => (
            <button 
              key={tab.id}
        ...
        {activeTab === 'gallery' && (
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {events.map(event => (
                  <div key={event.id} className="border-2 border-purple-200 rounded-lg p-4">
            ...
            <div className="space-y-3">
              {messages.map(msg => (
                <div key={msg.id} className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-gray-700 mt-2 bg-white p-3 rounded">{msg.body}</p>
                </div>
              ))}
              {messages.length === 0 && (
